...
def generate_calls(n_events, n_cpus=-1, outfile='gen.root', process='WprimeToWZ_lept', pixels=25, imrange=1, pileup=0, pt_hat_min=100, pt_hat_max = 500, bosonmass=800):
	...
	# for a in events_per_core:
	# 	print ' '.join(b for b in _generate_call(a))
...
def excecute_call(f):
	...
	# return subprocess.call(f, stdin=PIPE, stdout=DEVNULL, stderr=STDOUT)
	return subprocess.call(f, stdin=PIPE, stderr=STDOUT)




# _ = parallel_run(excecute_call, generate_calls(100, outfile='./testfiles/test.root'))
